...
emptyBoard :: [[Square]]
emptyBoard = replicate size $ replicate size sq
  where
    sq = Possible [1 .. size]
...
allCombs :: [([Int], Int)]
allCombs = map (\x -> (x, visibleScrapers x)) $ permutations [1 .. size]
...
hintIdx2Coord i
  | i >= 0 && i < size = zip (repeat y) [0 .. (size - 1)]
  | i >= size && i < (size * 2) = reverse $ zip [0 .. (size - 1)] (repeat y)
  | i >= (size * 2) && i < (size * 3) =
    reverse $ zip (repeat z) [0 .. (size - 1)]
  | otherwise = zip [0 .. (size - 1)] (repeat z)
  where
    y = i `mod` size
    z = size - 1 - y
...
boardRowFromI board i
  | i >= 0 && i < size = transpose board !! i
  | i >= size && i < (size * 2) = reverse $ board !! y
  | i >= (size * 2) && i < (size * 3) = reverse $ transpose board !! z
  | i >= (size * 3) && i < (size * 4) = board !! z
  where
    y = i `mod` size
    z = size - 1 - y
...
fixRow row = foldl fixRowNum row [1 .. size]
